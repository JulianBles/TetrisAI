...
class Tetris(Env):
    def __init__(self, height, width):
        ...
        self.action_space = spaces.Discrete(6) # Left, right, soft drop, hard drop, rotate clockwise, rotate counter-clockwise

        self.nr_of_drop_states = 6
        self.nr_of_actions = 6
        self.nr_of_pieces = 7
        # Drop state = punishment, nothing, 1 row, 2, 3, 4
        q_table = np.zeros([self.nr_of_drop_states * self.nr_of_pieces, self.nr_of_actions]) # Drop state TIMES the nr of different pieces
    ...
    def calculate_game_state(self):
        ...
        self.game_state = self.nr_of_drop_states * self.figure.type + drop_state
...
while not done:
    ...
    if counter % (fps // game.level // 2) == 0 or pressing_down:
        if game.state == "start":
            ...
            with open("tetris_AI_V2.json", "a") as file:
